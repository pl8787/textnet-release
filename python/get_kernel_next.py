...
for i in range(featmap_node.shape[0]):
    ...
    X = featmap_node[i]
    kernel_size = int((X.shape[0]/channel)**0.5)
    X = np.reshape(X, (channel, kernel_size, kernel_size))
    for j in range(channel):
      e = get_energy(X[j], (kernel_size, kernel_size))    
      file_name = '/'.join( [output_dir, \
        '%s_%s_%s_%s.jpg'%(node_idx, i, j, e)] )
      draw_map(X[j], (kernel_size, kernel_size), file_name)
