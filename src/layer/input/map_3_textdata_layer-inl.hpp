#ifndef TEXTNET_LAYER_MAP_3_TEXTDATA_LAYER_INL_HPP_
#define TEXTNET_LAYER_MAP_3_TEXTDATA_LAYER_INL_HPP_
...
class Map3TextDataLayer : public Layer<xpu>{
 public:
  Map3TextDataLayer(LayerType type) { this->layer_type = type; }
  virtual ~Map3TextDataLayer(void) {}
  ...
  virtual void SetupLayer(std::map<std::string, SettingV> &setting,
    ...
    utils::Check(bottom.size() == BottomNodeNum(),
                  "Map3TextDataLayer:bottom size problem."); 
    utils::Check(top.size() == TopNodeNum(),
                  "Map3TextDataLayer:top size problem.");
    ...
    if (mode == "pair") {
      MakePairs(rel_set, label_set, pair_set, loss_weight);
    } else if (mode == "list") {
  ...
  void ReadTextData(string &data_file, unordered_map<string, vector<int> > &data_set, 
    ...
    int s_len;
    int value;
    utils::Check(fin.is_open(), "Map3TextDataLayer: Open data file problem.");
    ...
    while (!fin.eof()) {
      ...
      utils::Check(data_set[key].size() >= min_doc_len, 
              "Map3TextDataLayer: Doc length %d less than %d, on line %d.", data_set[key].size(), min_doc_len, data_set.size());
    }
    fin.close();
  ...
  void MakePairs(vector<vector<string> > &rel_set, vector<int> &label_set, vector<vector<int> > &pair_set, unordered_map<string, float> &loss_weight) {
    int list_count = 0;
    string cur_data1 = "";
    ...
    for (auto it = loss_weight.begin(); it != loss_weight.end(); ++it) {
  ...
  void MakeInstances(vector<vector<string> > &rel_set, vector<int> &label_set, 
    ...
    for (int i = 0; i < line_count; ++i) {
      utils::Check(label_set[i] == 1, "Map3TextDataLayer: only support one label.");
      // store data1
  ...
  virtual void Reshape(const std::vector<Node<xpu>*> &bottom,
    ...
    utils::Check(batch_size > 0, "Map3TextDataLayer:batch_size <= 0");
    utils::Check(max_doc1_len > 0, "Map3TextDataLayer:max_doc1_len <= 0");
    utils::Check(max_doc2_len > 0, "Map3TextDataLayer:max_doc2_len <= 0");
    ...
    if (mode == "batch") {
      ...
      top[3]->Resize(batch_size, 1, 1, 1, true);
    } else if (mode == "pair") {
      ...
      top[3]->Resize(2 * batch_size, 1, 1, 1, true);
    } else if (mode == "list") {
      ...
      top[3]->Resize(factor * (batch_size-1) * batch_size, 1, 1, 1, true);
    } else if (mode == "inner_list") {
      ...
      top[3]->Resize((factor * (batch_size-1) + 1) * batch_size, 1, 1, 1, true);
    }
  ...
  virtual void Forward(const std::vector<Node<xpu>*> &bottom,
    ...
    mshadow::Tensor<xpu, 1> top3_data = top[3]->data_d1();
 ...
 protected:
  ...
  unordered_map<string, float> loss_weight;
...
template<typename xpu> unordered_map<string, vector<int> > Map3TextDataLayer<xpu>::data1_set = unordered_map<string, vector<int> >();
template<typename xpu> unordered_map<string, vector<int> > Map3TextDataLayer<xpu>::data2_set = unordered_map<string, vector<int> >();
}  // namespace layer
}  // namespace textnet
#endif  // LAYER_MAP_3_TEXTDATA_LAYER_INL_HPP_
