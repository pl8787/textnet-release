#ifndef TEXTNET_LAYER_GATE_LAYER_INL_HPP_
#define TEXTNET_LAYER_GATE_LAYER_INL_HPP_
...
class GateLayer : public Layer<xpu> {
 public:
  GateLayer(LayerType type) { this->layer_type = type; }
  virtual ~GateLayer(void) {}
  ...
  virtual void SetupLayer(std::map<std::string, SettingV> &setting,
    ...
    utils::Check(bottom.size() == BottomNodeNum(), "GateLayer:bottom size problem."); 
    utils::Check(top.size() == TopNodeNum(), "GateLayer:top size problem.");
                            
    no_bias = setting["no_bias"].bVal();
    num_input = bottom[0]->data.size(3);

    this->params.resize(2);
    this->params[0].Resize(num_input, 1, 1, 1, true);
  ...
  virtual void Reshape(const std::vector<Node<xpu>*> &bottom,
    ...
    gate.Resize(batch_size, num_seq, max_length, 1, true); 
  ...
  virtual void Backprop(const std::vector<Node<xpu>*> &bottom,
    ...
    for (int i = 0; i < top_diff.size(0); ++i) {
      for (int j = 0; j < top_diff.size(1); ++j) {
        gate_diff[i][0] += top_diff[i][j] * bottom_data[i][j] * 2;
      }
    }
    gate_diff *= F<op::sigmoid_grad>(gate_data);
    
    for (int i = 0; i < top_diff.size(0); ++i) {
        bottom_diff[i] += top_diff[i] * gate_data[i][0] * 2;
    }
