...
class MaxRecurrentLayer : public Layer<xpu> {
 public:
  ...
  virtual void ForwardOneStep(Tensor2D pre_h,
      ...
      // max_pooling rnn
      maxPooling(cur_h, x, pre_h, pos);
  }
  void maxPooling(Tensor2D p, Tensor2D c1, Tensor2D c2, Tensor2D pos) {
    utils::Check(p.size(0) == 1 && c1.size(0) == 1 && c2.size(0) == 1 && pos.size(0) == 1, 
                 "MaxRecurrent: pooling io size error");
    utils::Check(p.size(1) == c1.size(1) && p.size(1) == c2.size(1) && p.size(1) == pos.size(1),
                 "MaxRecurrent: pooling io size error");
  ...
  void unMaxPooling(Tensor2D p, Tensor2D c1, Tensor2D c2, Tensor2D pos) {
    utils::Check(p.size(0) == 1 && c1.size(0) == 1 && c2.size(0) == 1 && pos.size(0) == 1, 
                 "MaxRecurrent: pooling io size error");
    utils::Check(p.size(1) == c1.size(1) && p.size(1) == c2.size(1) && p.size(1) == pos.size(1),
                 "MaxRecurrent: pooling io size error");
  ...
  void ForwardLeft2Right(Tensor2D in, Tensor2D out, Tensor2D pos) {
  ...
  void ForwardRight2Left(Tensor2D in, Tensor2D out, Tensor2D pos) {
      int begin = 0, end = in.size(0);
      Tensor2D pre_h;
      // not need any padding, begin h and c are set to 0
  ...
  void BpOneStep(Tensor2D cur_h_er,
    ...
    unMaxPooling(cur_h_er, x_er, pre_h_er, pos);
    if (nonlinear_type == "sigmoid") {
