#ifndef TEXTNET_LAYER_FILL_CURVE_D2XY_LAYER_INL_HPP_
#define TEXTNET_LAYER_FILL_CURVE_D2XY_LAYER_INL_HPP_
...
class FillCurveD2XYLayer : public Layer<xpu>{
 public:
  FillCurveD2XYLayer(LayerType type) { this->layer_type = type; }
  virtual ~FillCurveD2XYLayer(void) {}
  ...
  virtual void SetupLayer(std::map<std::string, SettingV> &setting,
    ...
    utils::Check(bottom.size() == BottomNodeNum(),
                  "FillCurveD2XYLayer:bottom size problem."); 
    utils::Check(top.size() == TopNodeNum(),
                  "FillCurveD2XYLayer:top size problem.");    
    
    curve_type = setting["curve_type"].sVal();

    utils::Check(curve_type == "Hilbert" || curve_type == "Zigzag" || curve_type == "Peano", "FillCurveD2XYLayer: CurveD2XY Type Error.");
  }
  ...
  virtual void Reshape(const std::vector<Node<xpu>*> &bottom,
    ...
    int max_n = sqrt(bottom_shape[2]);
    top_shape[0] = bottom_shape[0];
    top_shape[1] = bottom_shape[3];
    top_shape[2] = max_n;
    top_shape[3] = max_n;
...
#endif  // LAYER_FILL_CURVE_D2XY_LAYER_INL_HPP_
