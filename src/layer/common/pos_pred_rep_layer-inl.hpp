...
class PosPredRepLayer : public Layer<xpu>{
 public:
  ...
  virtual void Reshape(const std::vector<Node<xpu>*> &bottom,
    ...
    utils::Check(shape_in_pos.size(2) == 1 && shape_in_pos.size(3) == 1, "PosPredRepLayer: shape error.");
    int feat_size = shape_in_l_rep.size(3);
  ...
  virtual void Forward(const std::vector<Node<xpu>*> &bottom,
    ...
    mshadow::Tensor<xpu, 4> top   = top[0]->data;

    utils::Check(l_len == r_len, "PosPredRepLayer: length error.");
    ...
    for (index_t batch_idx = 0; batch_idx < pos.size(0); ++batch_idx) {
      for (index_t pos_idx = 0; pos_idx < pos.size(1); ++pos_idx) {
        ...
        int p = pos[batch_idx][batch_idx][0][0];
        utils::Check(p < length, "PosPredRepLayer: position error.");
        if (p > 0) {
          top[batch_idx][pos_idx][0].Slice(0, feat_size) = F<op::identity>(l_rep[batch_idx][0][p-1]);
        }
        if (p < length-1) {
          top[batch_idx][pos_idx][0].Slice(feat_size, feat_size*2) = F<op::identity>(r_rep[batch_idx][0][p+1]);
        }
      }
    }
  }
