...
Updater<xpu, dim>* CreateUpdater_(UpdaterType type, std::map<std::string, SettingV> &setting, 
  ...
  switch(type) {
    ...
    case kAdaDelta: return new AdaDeltaUpdater<xpu, dim>(setting, prnd);
    case kSGDStep: return new SGDStepUpdater<xpu, dim>(setting, prnd);
    case kAdam: return new AdamUpdater<xpu, dim>(setting, prnd);
