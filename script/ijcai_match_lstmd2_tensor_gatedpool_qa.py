...
def gen_match_lstm(d_mem, init, lr, dataset, l2):
    ...
    net_cfg_train["max_iters"] = int(ds.train_max_iters * 1.5)
    ...
    layer['top_nodes'] = ['dpool_rep']
    layer['layer_name'] = 'dynamic_pooling'
    layer['layer_type'] = 10009
    setting = copy.deepcopy(g_layer_setting)
    layer['setting'] = setting
    setting['row'] = 5
    setting['col'] = 5
    setting['is_var_len'] = True
                ...
                for init in [0.3, 0.1]:
                    for lr in [0.4, 0.2, 0.1, 0.05]:
                        ...
                        for t_init_mul in [1]:
                            ...
                            net['log'] = 'log.match.lstmd2.gate_dpool.{0}.d{1}.run{2}.{3}'.format\
                            ...
                            gen_conf_file(net, '/home/wsx/exp/match/{0}/lstmd2/gate_dpool/run.{1}/'.format(dataset,str(run)) + \
                                               'model.match.lstmd2.gate_dpool.{0}.d{1}.run{2}.{3}'.format\
                                               (dataset, str(d_mem), str(run), str(idx)))
                            idx += 1
