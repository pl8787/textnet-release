...
def gen_conv_bilstm(d_mem, init, l2, lr, dataset):
    net = {}
    # dataset = 'tb_fine'
    if dataset == 'mr':
    ...
    # layer['top_nodes'] = ['drop_rep_2']
    # layer['layer_name'] = 'dropout_2'
    # layer['layer_type'] =  13
    # setting = {'rate':ds.dp_rate}
    # layer['setting'] = setting
    ...
    for d_mem in [50]:
        ...
        for init in [0.3, 0.1, 0.03, 0.01]:
            for lr in [0.3, 0.1, 0.03]:
                net = gen_conv_bilstm(d_mem = d_mem, init = init, lr =lr, dataset=dataset, l2 = 0.)
                net['log'] = 'log.conv_bilstm.max.dropbeforepool.{0}.d{1}.{2}'.format(dataset, str(d_mem), str(idx))
                # gen_conf_file(net, '/home/wsx/exp/tb/log/run.3/bilstm.max.tb_fine.model.' + str(idx))
                gen_conf_file(net, '/home/wsx/exp/gate/lstm/run.6/model.conv_bilstm.max.dropbeforepool.{0}.d{1}.{2}'.format(dataset, str(d_mem), str(idx)))
                idx += 1
