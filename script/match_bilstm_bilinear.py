...
def gen_match_lstm(d_mem, init, lr, dataset):
    ...
    layer = {}
    layers.append(layer) 
    layer['bottom_nodes'] = ['bi_pool_rep_drop']
    layer['top_nodes'] = ['softmax_prob']
    layer['layer_name'] = 'softmax_bilinear'
    layer['layer_type'] = 30 
    setting = copy.deepcopy(g_layer_setting)
    layer['setting'] = setting
    ...
    for d_mem in [50]:
        ...
        for init in [0.2, 0.07]:
            for lr in [0.5, 0.3, 0.1]:
                ...
                net['log'] = 'log.match.bilstm_bilinear.max.{0}.d{1}.{2}'.format(dataset, str(d_mem), str(idx))
                ...
                gen_conf_file(net, '/home/wsx/exp/match/bilstm_bilinear/run.1/model.match.bilstm_bilinear.max.{0}.d{1}.{2}'.format(dataset, str(d_mem), str(idx)))
                idx += 1
