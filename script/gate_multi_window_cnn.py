...
def gen_gate_cnn(d_mem, init, l2_gate = 0., dataset = 0):
    net = {}
    # dataset = 'tb_fine'
    norm2 = 9
    # dataset = 'mr'
    ...
    layer['top_nodes'] = ['conv_ret_trans_5']
    layer['layer_name'] = 'conv_ret_transform_5'
    layer['layer_type'] =  32 
    setting = {"phrase_type":2}
    layer['setting'] = setting
    ...
    layer = {}
    layers.append(layer) 
    layer['bottom_nodes'] = ['conv_ret_trans_5']
    layer['top_nodes'] = ['gate_result_5']
    layer['layer_name'] = 'gate_5'
    print "ORC: gate on each dim"
    layer['layer_type'] =  34 
    setting = copy.deepcopy(g_layer_setting)
    layer['setting'] = setting
    ...
    layer['top_nodes'] = ['conv_ret_trans_4']
    layer['layer_name'] = 'conv_ret_transform_4'
    layer['layer_type'] =  32 
    setting = {"phrase_type":2}
    layer['setting'] = setting
    ...
    layer = {}
    layers.append(layer) 
    layer['bottom_nodes'] = ['conv_ret_trans_4']
    layer['top_nodes'] = ['gate_result_4']
    layer['layer_name'] = 'gate_4'
    print "ORC: gate on each dim"
    layer['layer_type'] =  34 
    setting = copy.deepcopy(g_layer_setting)
    layer['setting'] = setting
    ...
    layer = {}
    layers.append(layer) 
    layer['bottom_nodes'] = ['gate_result_4']
    ...
    layer['top_nodes'] = ['conv_ret_trans_3']
    layer['layer_name'] = 'conv_ret_transform_3'
    layer['layer_type'] =  32 
    setting = {"phrase_type":2}
    layer['setting'] = setting
    ...
    layer = {}
    layers.append(layer) 
    layer['bottom_nodes'] = ['conv_ret_trans_3']
    layer['top_nodes'] = ['gate_result_3']
    layer['layer_name'] = 'gate_3'
    print "ORC: gate on each dim"
    layer['layer_type'] =  34 
    setting = copy.deepcopy(g_layer_setting)
    layer['setting'] = setting
    ...
    layer = {}
    layers.append(layer) 
    layer['bottom_nodes'] = ['gate_result_3']
...
for dataset in ['mr', 'tb_fine', 'tb_binary']:
    idx = 0
    for d_mem in [100]:
        for l2_gate in [0., 0.000001, 0.000003, 0.00001, 0.00003, 0.0001]:
            net = gen_gate_cnn(d_mem = d_mem, init = init, l2_gate = l2_gate, dataset = dataset)
            net['log'] = 'log.gate_all_mul_cnn.{0}.kim.ave.{1}'.format(dataset, str(idx))
            gen_conf_file(net, '/home/wsx/exp/gate/run.9/gate_all_mul_cnn.{0}.model.kim.ave.{1}'.format(dataset, str(idx)))
            idx += 1
